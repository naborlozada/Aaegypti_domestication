...
time java -Djava.io.tmpdir=${tmpdir} -Xmx8G -XX:+UseParallelGC -XX:ParallelGCThreads=1 -jar $GATK \
    ...
    --num_threads 1 \
    -o $WORKING_DIRECTORY/Population.variants.raw.vcf
...
# compress VCF file
# -----------------------------------------------------------------------------
bgzip -c $WORKING_DIRECTORY/Population.variants.raw.vcf > $WORKING_DIRECTORY/Population.variants.raw.vcf.gz
wait;

# index
bcftools index -t $WORKING_DIRECTORY/Population.variants.raw.vcf.gz
wait;




# Remove SNPs with a close proximity to indels of 10 bp
# -----------------------------------------------------------------------------
bcftools filter --threads 3 --SnpGap 10 -Oz -o $WORKING_DIRECTORY/Population.variants.raw.fltrd_close_proximity.vcf.gz  $WORKING_DIRECTORY/Population.variants.raw.vcf.gz
wait;
...
time java -Djava.io.tmpdir=${tmpdir} -Xmx8G -XX:+UseParallelGC -XX:ParallelGCThreads=1 -jar $GATK \
    -T SelectVariants \
    -R $refGENOME \
    -V $WORKING_DIRECTORY/Population.variants.raw.fltrd_close_proximity.vcf.gz \
    -selectType SNP \
    -o $WORKING_DIRECTORY/Population.snps.raw.vcf
wait;
...
# compress file
# -----------------------------------------------------------------------------
bgzip -c $WORKING_DIRECTORY/Population.snps.raw.vcf > $WORKING_DIRECTORY/Population.snps.raw.vcf.gz
wait;

# index
bcftools index -t $WORKING_DIRECTORY/Population.snps.raw.vcf.gz
wait;
...
time java -Djava.io.tmpdir=${tmpdir} -Xmx8G -XX:+UseParallelGC -XX:ParallelGCThreads=1 -jar $GATK \
    -T VariantFiltration \
    -R $refGENOME \
    -V $WORKING_DIRECTORY/Population.snps.raw.vcf.gz \
    --filterExpression "QUAL < 19.9 || QD < 2.0 || FS > 60.0 || MQ < 19.9 || MQRankSum < -12.5 || ReadPosRankSum < -8.0 || SOR > 3.0" \ 
    --filterName "my_snp_filter" \ 
    -o $WORKING_DIRECTORY/Population.snps.hrd_fltrd_flags.vcf.gz
wait;
...
# Filter SNPs
# -----------------------------------------------------------------------------
bcftools view -f PASS  -Oz -o $WORKING_DIRECTORY/Population.snps.fltrd.vcf.gz  $WORKING_DIRECTORY/Population.snps.hrd_fltrd_flags.vcf.gz
wait;


# Extraction of biallelic snps
# -----------------------------------------------------------------------------
bcftools view -m2 -M2 -Oz -o $WORKING_DIRECTORY/Population.snps.fltrd.biallelic.vcf.gz  $WORKING_DIRECTORY/Population.snps.fltrd.vcf.gz
wait;

# remove tmp no compressed VCF files manually after a population variant calling run is completed.
#rm $WORKING_DIRECTORY/*.vcf
#wait;

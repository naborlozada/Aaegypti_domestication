...
time java -Djava.io.tmpdir=${tmpdir} -Xmx8G -XX:+UseParallelGC -XX:ParallelGCThreads=1 -jar $GATK \
    -T SelectVariants \
    -R $refGENOME \
    -V $WORKING_DIRECTORY/Population.vars.raw.vcf.gz \
    -selectType SNP \
...
# Filter SNPs
# -----------------------------------------------------------------------------
bcftools view -f PASS  -Oz -o $WORKING_DIRECTORY/Population.snps.hrd_fltrd_flags.vcf.gz  $WORKING_DIRECTORY/Population.snps.fltrd.vcf.gz
wait;
...
# remove tmp no compressed VCF files manually after a population run is completed.
